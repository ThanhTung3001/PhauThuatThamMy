var slide = {
        ...
        init: function (s) {
          ...
          if (type.indexOf('height') >= 0) {
            ...
            slideItem.find('img').on('load', function () {
              ...
              if (
                _this.closest('.swiper-slide').hasClass('swiper-slide-active')
              ) {
          ...
          if (type.indexOf('imgView') >= 0) {
            ...
            list.each(function () {
              ...
              if (!_this.hasClass('swiper-slide-duplicate')) {
                ...
                if (_this.find('img').length)
                  imgSrc.push(_this.find('img')[0].cloneNode(true));
              }
            });
            list.on('click', function () {
              ...
              if (
                vodPlayer.obj[vodId] &&
                vodPlayer.obj[vodId].state == 'played'
              ) {
          ...
          if (type.indexOf('count') >= 0) {
            (function () {
              ...
              var total = totalcount >= 10 ? totalcount : '0' + totalcount;
        ...
        slideChange: function () {
          ...
          if (type.indexOf('bg') >= 0) {
            setTimeout(function () {
              ...
              activeSlide.classList.contains('nav2')
                ? slideNav.classList.add('on')
          ...
          if (type.indexOf('look') >= 0) {
            setTimeout(function () {
              ...
              activeSlide.classList.contains('type2')
                ? showWrap.classList.add('type2')
                : showWrap.classList.remove('type2');
            }, 100);
          }
        },
        slideChangeTransitionEnd: function (s) {
          ...
          if (type.indexOf('vod') >= 0) {
            ...
            if (active != null) {
              ...
              var target =
                player.querySelector('video') || player.querySelector('iframe');
  ...
  create: function (id, idx) {
    ...
    if ($('#visualSlide').hasClass('swiper-autoplay-off')) {
      // 20210316 서브 메인 상단 자동 슬라이드 비적용 / 적용

      if (id == 'visualSlide')
        swiper = new Swiper(
          '#' + id + ' .slide-container',
          slide.opt(id, 'opt5 page nav count')
        );
    } else {
      if (id == 'visualSlide')
        swiper = new Swiper(
          '#' + id + ' .slide-container',
          slide.opt(id, 'opt1 page nav count auto')
        );
    }
    ...
    if (id == 'visualSlide6')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt9 nav auto')
      );
    if (id == 'visualSlide7')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt1 page nav count notouch auto')
      );
    if (id == 'visualSlide8')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt2 nav')
      );
    if (id == 'visualSlide9') {
    ...
    if (id == 'visualSlide10_2' || 'visualSlide10_3') {
      ...
      innerProList.each(function () {
        ...
        var brandSlideWrapper = $('#' + id).find(
          '.main-best-cont.on .swiper-wrapper'
        );
      ...
      $('#' + id)
        .closest('.dp-section')
        .find('.tit-tab-wrap .tit-tab a')
        .click(function (e) {
          e.preventDefault();

          $('#' + id)
            .closest('.dp-section')
            .find('.tit-tab-wrap .tit-tab')
            .removeClass('on');
          ...
          if (
            $('#' + id)
              .find('.main-best-cont.on .swiper-wrapper')
              .hasClass('slick-slider') == false
          ) {
            var brandSlideSlick = brandSlideWrapper.slick({
              draggable: true,
              autoplay: true,
              ...
              prevArrow: $('#' + id).find('.main-best-cont.on .slide-nav-prev'),
              appendDots: $('#' + id).find(
                '.main-best-cont.on .slide-pagination'
              ),
            });
    ...
    if (id == 'visualSlide13') {
      ...
      if (slideLeng > 1) {
        swiper = new Swiper(
          '#' + id + ' .slide-container',
          slide.opt(id, 'opt1 page auto count')
        );
    ...
    if (id == 'visualSlide15')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt12 page nav count auto')
      );
    ...
    if (id == 'layerPromoList')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt10 nav')
      );
    if (id == 'promoChoice')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt11 nav page')
      );

    if (id == 'newPdBanner')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt1 page')
      );
    if (id == 'newPdSlide')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt2 page nav')
      );
    if (id == 'banner1')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt1 page nav')
      );

    if (id == 'moreEvent')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt21 page nav')
      );
    ...
    if (id == 'pdList') {
      ...
      $('.content-response .tit-tab-wrap .tit-tab a').click(function (e) {
        ...
        $('.content-response .tit-tab-wrap .tit-tab a')
          .closest('.tit-tab')
          .removeClass('on');
        $(this).closest('.tit-tab').addClass('on');
      ...
      imgBox.each(function () {
        ...
        var img = _this.find('.img');
        var nav =
          '<div class="slide-nav type3"><button type="button" class="slide-nav-prev">이전</button><button type="button" class="slide-nav-next">다음</button></div>';
    ...
    if (id == 'quickViewHistory') {
      ...
      slideWrap.each(function (idx) {
        ...
        if (
          (_this.hasClass('list-type1') && list.length < 1) ||
          (_this.hasClass('list-type2') && list.length < 5)
        )
          nav.hide();
      });
    }
    if (id == 'reviewImg')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt5 page')
      );
    if (id == 'pdViewSlide') {
      $(document).ready(function () {
        swiper = new Swiper(
          '#' + id + ' .slide-container',
          slide.opt(id, 'opt14 page nav imgView vod')
        );
    ...
    if (id == 'productSlideList')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt12 page nav')
      );
    if (id == 'productSlideList2')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt12 page nav')
      );
    if (id == 'productSlideList3')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt12 page nav')
      );
    if (id == 'viewContentsList')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt15 nav page')
      );
    if (id == 'viewContentsList2')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt15 nav page')
      );
    ...
    if (id == 'layerViewImgs') {
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt13 page nav vod')
      );
      swiper.on('slideChange', function () {
        var active = $('#' + id).find('.swiper-slide-active');
        var player = active.find('.vod');
    ...
    if (id == 'styleHtop')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt5 nav page')
      );
    if (id == 'thumbnailSlide')
      swiper = new Swiper(
        '#' + id + ' .slide-container',
        slide.opt(id, 'opt20 nav page')
      );
  ...
  bind: function (selector, options) {
    ...
    if (selector.startsWith('#')) {
      if ($(selector).find('.swiper-slide').length) {
        $(selector).attr(
          'data-slide-length',
          $(selector).find('.swiper-slide').length
        );
      }
      ...
      $(selector).each(function () {
        ...
        that.data(
          'swiper',
          new Swiper('#' + that.attr('id') + ' .slide-container', swiperOption)
        );
      });
    }
  },
...
$('.mainSideCont .slick-slider').slick({
  infinite: true,
  arrows: false,
  slidesToShow: 1,
  slidesToScroll: 1,
...
$('.mainWrap .mainCon .list01 .txt-wrap ul li').click(function () {
  const index = $(this).index();
  $('.mainWrap .mainCon .list01 .txt-wrap ul li').removeClass('on');
  $('.mainWrap .mainCon .list01 .txt-wrap ul li').eq(index).addClass('on');
  $('.mainWrap .mainCon .list01 .txt-wrap .txt .item').removeClass('on');
  $('.mainWrap .mainCon .list01 .txt-wrap .txt .item').eq(index).addClass('on');
  $('.mainWrap .mainCon .list01 .txt-wrap .image .item').removeClass('on');
  $('.mainWrap .mainCon .list01 .txt-wrap .image .item')
    .eq(index)
    .addClass('on');
});
function toggleSiteWrap() {
  const $siteWrap = $('.header .headInner .site-wrap');

  $siteWrap.css('display') == 'none'
    ? $siteWrap.css('display', 'flex')
    : $siteWrap.css('display', 'none');
}
